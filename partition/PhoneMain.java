...
public class PhoneMain {
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
        args = new String[]{"E:\\input\\selfdefinepartition\\phone.txt", "E:\\input\\selfdefinepartition\\partition\\llalalagain"};
        Configuration conf = new Configuration();
        Job job = Job.getInstance(conf);

        job.setJarByClass(PhoneMain.class);

        //connect map and reduce
        job.setMapperClass(PhoneMap.class);
        job.setReducerClass(PhoneReduce.class);
        job.setPartitionerClass(PhonePartition.class);

        //connect out put type of map and reduce
